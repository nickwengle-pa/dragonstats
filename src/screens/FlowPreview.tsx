...
interface PracticePlay { data: PlaySubmitData; before: GameState; remembered: Record<string, TaggedPlayer>; }
...
export default function FlowPreview() {
  ...
  const record = (data: PlaySubmitData) => {
    setPlays(prev => [...prev, { data, before: situation, remembered }]);
    ...
    setSituation(prev => ({ ...prev, ...next,
      ourScore: prev.ourScore + (data.isTouchdown && prev.possession === "us" ? 6 : 0),
      theirScore: prev.theirScore + (data.isTouchdown && prev.possession === "them" ? 6 : 0),
    }));
    setRemembered(prev => {
      const updated = { ...prev };
  ...
  return <main className="min-h-dvh bg-surface-bg text-slate-100 p-4 sm:p-6 max-w-4xl mx-auto space-y-5">
    ...
    <div className="flex flex-wrap items-center gap-3">
      <button className="btn-ghost min-h-11 px-3 border border-surface-border rounded-lg" onClick={() => setSituation(prev => ({ ...prev, possession: prev.possession === "us" ? "them" : "us", down: 1, distance: 10, ballOn: 35 }))}>Switch possession</button>
    ...
    <div className="card p-3">
      <QuickActions possession={situation.possession} progName="Dragons" oppName="Visitors" down={situation.down} distance={situation.distance} ballOn={situation.ballOn} spotLabel={spot(situation.ballOn)} onSelect={setPlayType} />
    </div>
    <section className="space-y-2">
      <div className="flex justify-between items-center"><h2 className="font-bold">Practice plays ({plays.length})</h2><button disabled={!plays.length} className="btn-ghost min-h-11 px-3 disabled:opacity-30" onClick={() => {
        ...
        setSituation(last.before); setRemembered(last.remembered); setPlays(prev => prev.slice(0, -1));
