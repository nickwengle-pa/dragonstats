...
export function SeasonReportDocument({ program, title, sections, loading = false, error = "", record, onBack, preview = false }: {
  ...
  return <div ref={screenRoot} className="min-h-dvh bg-surface-bg print:bg-white">
    <div className="p-5 flex items-center gap-4 print:hidden"><button onClick={onBack}>← {preview ? "Game Preview" : "Season Stats"}</button><h1 className="flex-1 font-bold">Season Report{preview ? " · Sample Preview" : ""}</h1><PrintReportButton root={() => screenRoot.current} filename={pdfFilename(`${program?.abbreviation ?? "Team"} ${title} Season Stats`)} className="btn-primary" disabled={loading || !!error}>Print / Save PDF</PrintReportButton></div>
