...
export default function UiPreview() {
  ...
  return <main className="ui-preview">
    ...
    <div className="ui-field"><FieldVisualizer ballOn={draft.spotted ? end : state.ballOn} ballPosition={right ? (draft.spotted ? end : state.ballOn) : 100 - (draft.spotted ? end : state.ballOn)} firstDownPosition={right ? Math.min(100, state.ballOn + state.distance) : Math.max(0, 100 - state.ballOn - state.distance)} possession={state.possession} ourEndZoneSide={flipped ? "right" : "left"} primaryColor="#dc2626" progName="Purchase Line" oppName="Northern Cambria" progAbbr="PL" oppAbbr="NC" oppColor="#b89b24" onFlipDirection={() => setFlipped(f => !f)} onPickSpot={n => yards(Math.round(right ? n : 100 - n) - state.ballOn)} /></div>
    <div className="ui-field-caption">Tap the field to set the ending spot <span>Snap: {spot(state.ballOn)}{draft.spotted ? ` → End: ${spot(end)}` : ""}</span></div>
