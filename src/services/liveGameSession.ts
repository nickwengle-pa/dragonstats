...
function buildFumble(play: PlayRecord, ballCarrier: string, config: LiveSessionConfig): FumbleEvent | undefined {
  ...
  return {
    ...
    recoveryYards: play.fumbleReturnYards ?? undefined,
    isTouchdown: play.isTouchdown,
  };
}
...
function toEnginePlay(
  ...
  // The penalty replaces the play (roughing the kicker, or the operator's
  // override). Types delegated to convertPlay above get the same rule there.
  const playCounts = play.playData?.penalty_play_counts;
  if (penaltyWipesPlay(play.type, play.penalty, play.penaltyCategory, play.penaltyEnforcement,
    typeof playCounts === "boolean" ? playCounts : null)) {
  ...
  switch (play.type) {
    case "rush": {
      ...
      return {
        type: PlayType.Rush,
        rusher,
        result: play.isTouchdown && !play.turnover ? RushResult.Touchdown : RushResult.Normal,
        yardsGained: play.yards,
        isTouchdown: play.isTouchdown && !play.turnover,
        tackledBy: tackles.tackledBy,
        assistedTackle: tackles.assistedTackle,
        fumble: buildFumble(play, rusher, config),
        returnTackledBy: playersByRole(play, "recovery_tackler"),
        penalties,
    ...
    case "sack": {
      ...
      return {
        ...
        yardsGained: play.yards,
        isTouchdown: false,
        fumble: buildFumble(play, passer, config),
        returnTackledBy: playersByRole(play, "recovery_tackler"),
    ...
    case "int": {
      ...
      return {
        ...
        returnTackledBy: [...(tackles.tackledBy ?? []), ...(tackles.assistedTackle ?? [])],
        penalties,
        description: play.description || undefined,
        context,
      } satisfies PassPlay & { context: PlayContext } as Play;
