...
-- (fixes Supabase linter CRITICAL "Security Definer View")
CREATE VIEW game_schedule WITH (security_invoker = true) AS
SELECT
  g.*,
...
CREATE VIEW player_season_summary WITH (security_invoker = true) AS
SELECT
  sr.player_id,
  p.first_name,
  p.last_name,
